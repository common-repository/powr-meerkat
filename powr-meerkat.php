    ...
    */
    ...
    class Powr_Meerkat extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Meerkat.
        </p>
        <p>
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_meerkat_menu' );
    function powr_meerkat_menu() {
      add_menu_page( 'POWr Meerkat', 'POWr Meerkat', 'manage_options', 'powr-meerkat-settings', 'powr_meerkat_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_meerkat_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-meerkat" src="https://www.powr.io/wp-create/meerkat" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-meerkat"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_meerkat_plugin_activate');
      add_action('admin_init', 'powr_meerkat_plugin_redirect');

      function powr_meerkat_plugin_activate() {
      add_option('powr_meerkat_plugin_do_activation_redirect', true);
      }

      function powr_meerkat_plugin_redirect() {
      if (get_option('powr_meerkat_plugin_do_activation_redirect', false)) {
          delete_option('powr_meerkat_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-meerkat-settings' );
          }
       }
      }
